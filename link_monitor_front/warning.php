...
<html>
<head>
        <meta http-equiv="refresh" content="6" charset="UTF-8">
...
$sql = 'select * from warn_log where log_content like "%down%" order by datetime desc limit 30';
// 结果集
...
for($i=0; $i<$newsNum; $i++){
    ...
    if (strpos($row['log_content'],'goback')) {
    echo "<tr>";
    echo "<td>{$row['id']}</td>";
    ...
    echo "</tr>";
    echo '<embed loop=true autostart=true starttime=00:00 volume=100 hidden=true src="http://map.efly.cc/ahah.mp3">';
         }
			      }
// 5. 释放结果集
...
<script type="text/javascript">
    function del (id) {
		  if (confirm("确定删除这条监控记录")){
		      window.location = "del_log_warn_record.php?id="+id;
		      }
		}
</script>
</body>
